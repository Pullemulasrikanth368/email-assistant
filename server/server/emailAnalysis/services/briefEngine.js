...
/**
 * Run a ALREADY-BUILT brief prompt through the shared aiClient and normalise
 * the result to the brief contract. Encapsulates the single AI call, the
 * usable-brief check and the one-shot completeness retry so any caller (daily
 * report OR pre-meeting brief) reuses the exact same execution path and
 * provider selection. THROWS on an unusable/unavailable response so the caller
 * can decide how to fall back.
 *
 * @param {string} prompt      - a full prompt (e.g. from buildBriefPrompt / buildPreMeetingPrompt)
