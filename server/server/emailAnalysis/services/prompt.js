...
13. categorySummaries: group the emails by their "category" field (emails with no category go under "Other"). For EACH distinct category produce { "category", "count", "summary", "keyPoints", "mails", "groups" }:
    ...
    - "groups": REQUIRED for EVERY category that has at least one email — never omit it and never return an empty array. This applies to ALL categories equally (Personal, Travel, newsletters, promotions, Other — no exceptions). Cluster the SAME category's emails into topic groups (Gmail-style) by actually READING each email's body/content, not just its subject line:
        ...
        * "email_ids": [sourceId, ...] — the sourceIds (from this category's "mails") backing this group. EVERY email in the category must appear in EXACTLY ONE group; never leave a mail out and never place a mail in two groups. Decide grouping DYNAMICALLY by reading each email's subject, sender and body to infer its underlying subject matter/entity/service/account/thread/recurring stream, then put emails that share the same underlying subject matter into ONE group — even when subjects, dates, amounts or reference numbers differ. Do NOT split recurring/periodic items of the same kind by month, date, period or reference number. Prefer FEWER, broader groups; only split when the subject matter is genuinely different. A truly unrelated email is its own single-email group.
...
RULES:
- Decide grouping DYNAMICALLY: read each email's subject, sender AND body and infer what it is genuinely about — its underlying subject matter, entity, service, vendor, account, order, project, event, conversation thread, or recurring stream. Never judge by the subject wording alone.
- Put every email that shares the SAME underlying subject matter into ONE group, even when their subjects, dates, amounts, or reference numbers differ. Recurring or periodic items of the same kind (successive statements/bills, repeated notifications from the same source, follow-ups and updates on the same matter) all go in ONE group — do NOT split them by month, date, period, sequence, or reference number.
- Prefer FEWER, BROADER groups. Only create separate groups when the emails are about genuinely DIFFERENT subject matter.
- A genuinely unrelated email is its own single-email group.
...
- "group_title": a SHORT headline naming the shared subject matter / entity of the group, inferred from the content (not a per-item, per-month or per-reference title).
- "summary": 1-2 COMPLETE, easy-to-understand sentences that MERGE the content of ALL emails in the group so the reader understands them without opening any. When the group spans several items or periods, state EACH one's key detail in the same summary (amounts, dates, names, numbers, places). Never truncate or end with an ellipsis. You MAY use only these inline tags: <b>…</b>, <mark>…</mark>, <span class="danger">…</span>.
- "email_ids": the "id" values of ALL emails that belong to this group.
